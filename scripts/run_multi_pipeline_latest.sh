...
CARTESIAN_MIN_FRACTION="${CARTESIAN_MIN_FRACTION:-0.0}"
...
update_capture_config() {
  ...
  # 2. Update the file safely for manual script executions
  python3 - <<PY
from pathlib import Path
import re
...
# Update TEST_NAME
s = re.sub(r'^TEST_NAME\s*=.*$', f'TEST_NAME = "{TEST_NAME}"', s, flags=re.M)

# Update STEP_INDEX safely by replacing the entire line
if 'STEP_INDEX =' in s:
    s = re.sub(r'^STEP_INDEX\s*=.*$', f'STEP_INDEX = int(os.getenv("STEP_INDEX", "{step_index}"))', s, flags=re.M)
